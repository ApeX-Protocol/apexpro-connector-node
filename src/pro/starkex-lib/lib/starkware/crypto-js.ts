 ...
 */
export function pedersen(...input: [BN, BN]): BN {
...
export function sign(ecKeyPair: EllipticCurve.KeyPair, messageHash: BN): EllipticCurve.Signature {
...
function bnInRange(input: BN, lowerBoundInclusive: BN, upperBoundExclusive: BN): boolean {
  return input.gte(lowerBoundInclusive) && input.lt(upperBoundExclusive);
}

function fixHashLength(messageHash: BN): BN {
